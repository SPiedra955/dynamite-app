      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-16 text-center">
          ...
          <p className="mt-4 text-gray-400">
            Genera tu plan de dieta y ejercicio personalizado con IA
          </p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className="flex h-full flex-col rounded-2xl border border-gray-700 bg-gray-900 p-8 shadow-lg transition duration-300 hover:-translate-y-2 hover:border-red-500"
            >
              <h5 className="text-2xl font-semibold text-white">
                {plan.name}
              </h5>

              <div className="my-4 h-1 w-16 rounded-full bg-red-600"></div>
              ...
              <button
                className="mt-auto w-full rounded-full bg-red-600 px-6 py-3 font-semibold text-white transition hover:bg-red-700"
                onClick={async () => {
                  if (!token) {
                    navigate("/authentication");
                    return;
                  ...
                  try {
                    const res = await fetch(
                      `${url}/api/create-subscription-checkout`,
                      {
                        method: "POST",
                        headers: {
                          Authorization: `Bearer ${token}`,
