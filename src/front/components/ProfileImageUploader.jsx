...
export function ProfileImageUploader() {
  ...
  const [imageUrl, setImageUrl] = useState(store.user?.photo || "");

  const extractPublicId = (url) => {
  if (!url) return "";
  const parts = url.split("/upload/");
  if (parts.length < 2) return "";
  // Quita los parámetros de transformación (f_auto,q_auto/v1/) y devuelve solo el id
  const afterUpload = parts[1];
  const match = afterUpload.match(/(?:[^/]+\/)*v\d+\/(.+)$/);
  return match ? match[1] : afterUpload;
  };

  const [publicId, setPublicId] = useState(extractPublicId(store.user?.photo || ""));
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch({ type: "set_profile_image", payload: { url: data.url, public_id: data.public_id } });

        // Guarda la foto en la base de datos
        const token = localStorage.getItem("token");
        const userId = store.user?.id;
        await fetch(`${backendUrl}/api/update/user/${userId}`, {
          method: "PUT",
          headers: {
  ...
  return (
    <>
      <style>{`
        ...
        .piu-badge:hover { background: #4338ca; }
        ...
        .piu-btn:active { transform: scale(0.97); }
        .piu-btn:disabled { opacity: 0.5; cursor: not-allowed; }

        .piu-btn-primary { background: #4f46e5; color: #fff; }
        .piu-btn-primary:hover:not(:disabled) { background: #4338ca; }

        .piu-btn-danger { background: #fff; color: #ef4444; border: 1.5px solid #fca5a5; }
        .piu-btn-danger:hover:not(:disabled) { background: #fef2f2; }
